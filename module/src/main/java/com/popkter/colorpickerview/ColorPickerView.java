...
public class ColorPickerView extends View {
    ...
    //饱和度倍数，0 - 1，饱和度越高颜色越艳丽，可根据具体需求调整
    ...
    private void init(AttributeSet attrs) {
        ...
        try {
            mBigCircle = types.getDimensionPixelOffset(R.styleable.color_picker_circle_radius, 100);
            mRudeRadius = types.getDimensionPixelOffset(R.styleable.color_picker_center_radius, 10);
            mCenterColor = types.getColor(R.styleable.color_picker_center_color, Color.WHITE);
            isRing = types.getBoolean(R.styleable.color_picker_is_ring, false);
            mRingCircle = types.getDimensionPixelOffset(R.styleable.color_picker_ring_radius, 30);
        } finally {
    ...
    private Bitmap createColorWheelBitmap(int width, int height) {
        ...
        if (isRing) {
            ...
            canvas.drawCircle(width / 2, height / 2, mBigCircle - mRingCircle, mBlankPaint);
        }
        return bitmap;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isRing) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    length = getLength(event.getX(), event.getY(), mCenterPoint.x, mCenterPoint.y);
        ...
        mHSV[1] = (float) sat;
        mHSV[2] = (float) brt;
        Log.d("iKotliner", "onTouchEvent: mHSV[0] = " + mHSV[0] + "  mHSB[1]=" + mHSV[1] + "  mHSB[2]==" + mHSV[2]);
